...
class FundraisersViewController: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource, UITextFieldDelegate {
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        ref5.observeSingleEvent(of: .value) { (DataSnapshot) in
            ...
            if currentFundraisers2.contains(self.fundraisers[indexPath.row].name) {
                // Go to FundraiserDetailVC
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "fundraisersVCToFundraiserDetailVCSegue" {
            let fundraiserDetailVC = segue.destination as! FundraiserDetailViewController
            fundraiserDetailVC.name = fundraiserDetailVCName
        }
    }
