...
function addPointsToUser(fundraiser, user) {
	...
	return query.once('value').then( function(querySnapshot) {
		...
		query.set(querySnapshot.val() + 1);
		//update fundraiser total raised	
		addOneBidToFundraiser(fundraiser); 
		return true; 
	}); 
}

function addOneBidToFundraiser(fundraiser) {
	var query = admin.database().ref('/fundraisers/' + fundraiser); 
	var totalRaisedQuery = admin.database().ref('/fundraisers/' + fundraiser + '/totalRaised/'); 
	return query.once('value').then( function(querySnapshot) {
		const bid = querySnapshot.child('bid').val(); 
		
		const totalRaised = querySnapshot.child('totalRaised').val();
		const newTotal = parseInt(bid) + parseInt(totalRaised); 
		
		totalRaisedQuery.set(newTotal);
		return true; 
	}); 
}
